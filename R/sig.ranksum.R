...
.sig.ranksum <- function(exprdata, up=NULL, dn=NULL, ns=NULL, full.return = FALSE)
{
	...
	if (length(up) != 0)
	{
		...
		ranksum <- rowSums(apply(dat, 1, function(x) {rank(x, "average", na.last="keep")}), na.rm=TRUE)
		col.counts <- colSums(!is.na(dat))
	}
	if (length(dn) != 0)
	{
		dat <- exprdata[dn, , drop = FALSE]
		ranksum <- ranksum + rowSums(ncol(exprdata) - apply(dat, 1, function(x) {rank(x, "average", na.last="keep")}) + 1, na.rm=TRUE)
	...
	gene.cor <- function(gene.idx, is.up, exprdata, pat.order)
	{
		...
		if (is.up == TRUE)
			gene.expr <- -gene.expr
		cor(gene.expr, 1:ncol(exprdata), method="spearman", use="pairwise")
	}
